 ...
 * 
 * ToDo:
 * - IntervalTree depends on SortedList
 *   https://github.com/shinout/interval-tree
 *   Rename SortedList into IntervalTree 
 ...
 *  - 
 *  	$.ajax({
        url: "pod.view.div3.json",
        type: "GET",
        crossDomain : true,
        dataType: "json",
        success: function (xmlLst) { console.log(xmlLst); }        		 
    });	
 *
 * 
 * @returns
...
function pvQueryAnalysis( qryObj ) {
	...
	for (let iCtrNat = 0; iCtrNat < Object.keys(pvQryObj[pvQryAtt1]).length; iCtrNat++) {		
		...
		for (let iCtrMtf = 0; iCtrMtf < Object.keys(pvQryObj[pvQryAtt1][iCtrNat][pvQryAtt2][pvQryAtt3]).length; iCtrMtf++) {			
			...
			strTxt   = pvDatObj[iCtrMtf].textPattern;			
			strStart = pvDatObj[iCtrMtf].start;								
			strEnd   = pvDatObj[iCtrMtf].end;
			strId    = pvDatObj[iCtrMtf].id;						
			strType  = pvDatObj[iCtrMtf].type;
